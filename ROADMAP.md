...
The tuning phase. Five of the six planned experiments ran (the sixth — hybrid/reranking/nDCG — was always a stretch and is punted to Future extensions); the winning configuration then shipped as the application default, and the README grew its portfolio sections. No new pipeline capability was built — the whole phase is *measurement plus the one config change the measurements justified*, which is what the two-tier eval harness from Phases 2–3 existed to enable. All 85 unit tests pass (8 new, from the threshold sweep's `tests/test_threshold.py`), ruff clean. Every experiment is a dated `change → hypothesis → before/after → conclusion` entry in EVALS.md — this section is the index and the meta-story, not a re-derivation of those numbers.

**The experiments ran in a deliberate dependency order, not the roadmap's list order.** Each one's outcome set the ground the next was measured against, so running them out of order would have tuned the wrong system:

1. **Embedding comparison first (roadmap exp 5).** `all-MiniLM-L6-v2` beats `nomic-embed-text` by **+16.7 pts recall@5** (0.5535 → 0.7209) on the local stack — replicating the Phase 5 CI finding exactly. This *had* to go first: every subsequent sweep is run against the winning embedder, and tuning chunk size or top-k on the weaker profile would have optimized a system we weren't going to ship. The portfolio lesson is the headline — a 384-dim CPU model beat the 768-dim GPU retrieval-specialist because *fit* (short scientific claim-vs-abstract matching, squarely in MiniLM's training distribution) beat *size*. MiniLM became the primary profile for everything downstream.
2. **Chunk-size / overlap sweep (exp 1).** Re-ingested at 100/20, 150/30, 300/60 (overlap fixed at 20%). **300/60 wins recall@5 (0.7510)** despite MiniLM's ~256-token truncation only embedding the opening ~200 words of each coarse chunk — evidence the openings carry most of SciFact's retrieval signal, and that at ~9-in-10 single-chunk documents the coarse config effectively retrieves whole abstracts. Recorded honestly with its confound (chunk-counted k against document-level labels mechanically favors coarser chunks). Adopted 300/60 as the corpus config for the rest of the phase.
3. **No-answer threshold sweep (exp 3).** Built `evals/run_threshold.py` / `evals/threshold.py` (+ unit tests) to sweep the false-refusal vs. OOD-refusal trade-off on *both* profiles' own similarity scales. Confirmed the scale is profile-specific: the inherited 0.60 would falsely refuse 116/300 (38.7%) in-domain queries on the MiniLM corpus. Landed **0.36 for MiniLM 300/60** (8/8 OOD caught at a nominal 2% false-refusal cost, where the "false" refusals are exactly the weakest-retrieval queries). This experiment ran *after* the chunk sweep on purpose — the threshold had to be tuned against the finally-adopted corpus, not a throwaway one.
4. **Top-k sensitivity (exp 2).** The one experiment that spans both tiers. Tier-1 recall@k keeps climbing (knee at k=5); Tier-2 faithfulness **peaks at k=5 (4.01) and drops at k=8 (3.62)** as the lower-similarity extra chunks dilute the generator's context — the phase's first ungrounded citation appeared at k=8. "More retrieved context" and "better answers" are shown to be different axes. Adopted `top_k=5` (already the default); the k=5 row is the new adopted-config Tier-2 baseline that supersedes the Phase 3 nomic 200/40 generation baseline.
5. **Generator prompt variant — claim reframing (exp 4, REJECTED).** The highest-leverage lever the Phase 3 baseline pointed at: reframe SciFact's declarative claims as "assess this statement" to attack the 64% refusal rate. It *worked* on the naive metric (answered 44% → 87%, refusals 161 → 33) and was **rejected anyway** — faithfulness collapsed 4.01 → 3.06, and the per-case split proved why: the 128 newly-answered queries (ones the generator used to refuse) scored **2.11/5**. The load-bearing conclusion for the whole project: **the high refusal rate was substantially *correct* refusal** — the retrieved abstract genuinely doesn't adjudicate those claims. This is the two-tier eval doing its defining job: catching a change that looks like a decisive win on refusal rate as a serious quality regression, and naming exactly which answers degraded. The baseline generator prompt stands.

**The profile switch — the one config change that shipped (commit `5cf5eae`).** Experiments 1/3 concluded that MiniLM/300-60/0.36 is the configuration to serve, but the *how* mattered: the winning corpus already lived in a sweep database, so re-ingesting from scratch would have been wasted work. Chosen approach ("Option C"): renamed the existing MiniLM 300/60 sweep DB to the canonical `RAG_Project_DB` and pointed the config defaults at it — no re-ingest. Code defaults moved in lockstep (`EMBEDDING_PROFILE=sentence-transformers`, `CHUNK_SIZE=300`/`OVERLAP=60`, `NO_ANSWER_THRESHOLD=0.36`) so the shipped defaults match the measured winner rather than living only in `.env`. Because the app now serves the CPU (sentence-transformers) profile, the **container had to change**: the Dockerfile now bakes the MiniLM weights into the image at build time and installs the CPU-profile runtime deps so the API runs the CPU profile fully self-contained and offline (no HuggingFace fetch at container start, no dependency on the host's model cache). Rebuilt and verified end to end. An `embedding.py` deprecation was fixed in passing.

**Cleanup (2026-07-22).** The sweep experiments left five databases in the pgvector volume. Dropped the three throwaway MiniLM sweep DBs (`_minilm`, `_minilm_c100o20`, `_minilm_c150o30`); **kept `RAG_Project_DB_nomic_archive`** deliberately — it's the original nomic-embed-text corpus, retained as a reproducibility artifact so the embedding-comparison and nomic-threshold numbers in EVALS.md can be re-run without a full re-ingest. Volume now holds only the active corpus and that archive.

**Documentation deliverables.** The README gained its portfolio sections (architecture overview, the code-first-vs-n8n comparison, the Guardrails section documenting all four built layers plus the one-line "what production adds," and the scale-up/queue-worker notes) plus a recall@k curve figure (retrieval side of exp 2), landed across commits `e0c8ff2` and `476f24f`. Security hardening rode along: `error_webhook_url` and other secret-bearing fields moved to pydantic `SecretStr` (closing the Phase 5 note about `Settings`'s repr echoing secrets into tracebacks). The close-out doc pass then (a) gave EVALS.md a narrative-coherence read-through — added a "Where the system landed" summary and updated the Corpus header to the shipped config, without touching any dated entry; (b) reconciled the README with the profile switch — it had described nomic as primary and claimed the portfolio numbers came from the Ollama profile, both now false, since the shipped and CI profiles are the same MiniLM; and (c) flipped the status block to "all phases complete."

**Deps rename (close-out).** The profile switch exposed a naming lie: `requirements-ci.txt` (torch + sentence-transformers) was named as if CI-only, but it is now the runtime the app, the container, *and* CI all need for the default CPU profile — while the README's Setup still installed the base `requirements.txt`, which lacks those deps (host-run `ingest.py`/`main.py`/evals would `ModuleNotFoundError` on the sentence-transformers import, which is lazy so `pytest`/`ruff` stayed green and hid it). Renamed `requirements-ci.txt` → `requirements-cpu.txt` (references updated in the Dockerfile and `ci.yml`; historical Phase 1/4/5 notes above keep the old name as the record of their time), and rewrote README Setup to install it by default with the lighter Ollama-profile path documented as the alternative. Also refreshed the now-stale `ci.yml` floor comment: CI inherits the config chunk defaults, so it now ingests at the shipped 300/60 (5,827 chunks) and measured recall@5 **0.751** (run 29889385589, 2026-07-22) — the 0.68 floor, calibrated 2026-07-18 against the old 200/40 corpus (0.7209), still holds conservatively and was left unchanged.

**README n8n section removed (2026-07-25, commit `cb05e4c`).** The code-first-vs-n8n comparison recorded above was cut from the README. It was the only section arguing a general tooling thesis rather than describing this system — it answered a question the README never raised, broke the document's noun-phrase heading pattern and descriptive register, and wedged an essay between Guardrails and Scaling up, two sections that pair naturally (both are "what this doesn't do yet and what would change"). Its one concrete, load-bearing detail — that the global exception handler with its short reference id and webhook alert is the n8n error-flow pattern translated into FastAPI — was folded into **Guardrails layer 4**, where it sits next to the code it describes rather than trailing a philosophy section as a parenthetical. `Constraints_and_context.md` was revised in step: the zero-n8n rule stands, but the README-comparison-section requirement it mandated does not. The deliverables line above is left as written — it records what Phase 6 planned and shipped at the time, not the current README.

**Deferred (now Future extensions, not this phase):** the stretch experiment 6 — hybrid retrieval (BM25 + vector), reranking, and nDCG. Left as a clean future goal rather than a half-built rush at the end of the phase.
